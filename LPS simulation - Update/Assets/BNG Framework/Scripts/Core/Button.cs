...
namespace BNG {
    ...
    public class Button : MonoBehaviour {
        ...
        List<HandIntegration> grabbers = new List<HandIntegration>(); // Grabbers in our trigger
        ...
        void Update() {
            ...
            for (int x = 0; x < grabbers.Count; x++) {
                if (!grabbers[x].gameObjectInHand) {
        ...
        void OnTriggerEnter(Collider other) {
            // Check Grabber
            HandIntegration grab = other.GetComponent<HandIntegration>();
            if (grab != null) {
                if (grabbers == null) {
                    grabbers = new List<HandIntegration>();
                }
