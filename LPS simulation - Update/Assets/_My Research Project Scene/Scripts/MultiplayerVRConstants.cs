
public class MultiplayerVRConstants
{
    public const string MAP_TYPE_KEY = "map";
    public const string MAP_TYPE_VALUE_TUTORIAL = "Tutorial";
    public const string MAP_TYPE_VALUE_EXPERIMENT = "Experiment";
    public const string MAP_TYPE_VALUE_EXPERIMENTTRADITIONAL = "Experiment Traditional";
    public const string MAP_TYPE_VALUE_WARMUP = "Warm Up";



    public const string AVATAR_SELECTION_NUMBER = "Avatar_Selection_Number";
}
