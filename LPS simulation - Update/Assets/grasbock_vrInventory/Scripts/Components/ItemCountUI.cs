...
namespace GVRI { 
    public class ItemCountUI : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!slot) return;
            //reposition
            if (target && BagInventory)
            {//there is a target to face towards
                origin.LookAt(target.transform);
            }
            else // only rotate with Y axies when it is not a bag inventory
            {
                Vector3 targetPostion = new Vector3(target.transform.position.x,
                    transform.position.y, target.transform.position.z);
                origin.LookAt(targetPostion);
            }
